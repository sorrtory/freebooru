...
<template>
  <main class="page resource-page">
    ...
    <section v-else aria-labelledby="collection-tags"><header><h2 id="collection-tags">In this collection</h2><span>{{ imported.length }}</span></header><div class="resource-list"><article v-for="tag in imported" :key="tag.name"><div><div class="tag-title"><RouterLink :to="{ path: `/collections/${encodeURIComponent(collection)}/files`, query: { q: tag.name } }">{{ tag.name }}</RouterLink><span v-for="group in tag.groups ?? []" :key="group">{{ group }}</span></div><p>{{ tag.comment || tag.type }}</p></div><div class="resource-meta"><span v-if="tag.required">Required</span><span>{{ tag.assignment_count }} files</span><span v-if="tag.values.length">{{ tag.values.length }} values</span></div></article><p v-if="!imported.length">No matching imported tags.</p></div></section>
...
.tag-title { display: flex; flex-wrap: wrap; align-items: center; gap: .35rem; }.tag-title a { color: var(--text); font-weight: 800; text-decoration: none; }.tag-title a:hover { color: var(--primary); text-decoration: underline; }.tag-title span { padding: .15rem .35rem; border: 1px solid var(--border); border-radius: 999px; color: var(--text-muted); font-size: .68rem; }
