...
const requiredOnly = shallowRef(route.query.required === 'true')
const loading = shallowRef(true)
const error = shallowRef('')
const importing = shallowRef('')
const showOther = shallowRef(false)
const imported = computed(() => tags.value.filter((tag) => tag.imported))
const available = computed(() => tags.value.filter((tag) => !tag.imported))

onMounted(load)

async function load() {
  loading.value = true; error.value = ''
  try { tags.value = await getCollectionTags(props.collection, query.value, requiredOnly.value) }
  catch (cause) { error.value = cause instanceof Error ? cause.message : 'Tags are unavailable.' }
  finally { loading.value = false }
}

async function filter() {
  await router.replace({ query: { ...(query.value ? { q: query.value } : {}), ...(requiredOnly.value ? { required: 'true' } : {}) } })
  await load()
}
...
<template>
  <main class="page resource-page">
    ...
    <form class="resource-filter" role="search" @submit.prevent="filter"><input v-model="query" type="search" placeholder="Find a tag"><label><input v-model="requiredOnly" type="checkbox"> Required only</label><button class="button" type="submit">Filter</button></form>
    <p v-if="error" class="resource-error" role="alert">{{ error }}</p><p v-if="loading" role="status">Loading tags…</p>
    <section v-else aria-labelledby="collection-tags"><header><h2 id="collection-tags">In this collection</h2><span>{{ imported.length }}</span></header><div class="resource-list"><article v-for="tag in imported" :key="tag.name"><div><strong>{{ tag.name }}</strong><p>{{ tag.comment || tag.type }}</p></div><div class="resource-meta"><span v-if="tag.required">Required</span><span>{{ tag.assignment_count }} files</span><span v-if="tag.values.length">{{ tag.values.length }} values</span></div></article><p v-if="!imported.length">No matching imported tags.</p></div></section>
    <section v-if="available.length" aria-labelledby="other-tags"><button class="section-toggle" type="button" :aria-expanded="showOther" @click="showOther = !showOther"><span><small>Available</small><strong id="other-tags">Other configured tags</strong></span><b>{{ available.length }} {{ showOther ? '−' : '+' }}</b></button><div v-if="showOther" class="resource-list"><article v-for="tag in available" :key="tag.name"><div><strong>{{ tag.name }}</strong><p>{{ tag.comment || tag.type }}</p></div><button class="button" type="button" :disabled="Boolean(importing)" @click="add(tag)">{{ importing === tag.name ? 'Importing…' : 'Import' }}</button></article></div></section>
  </main>
</template>
...
.resource-filter label { display: flex; min-height: 2.75rem; align-items: center; gap: .5rem; }
...
@media (min-width: 48rem) { .resource-filter { grid-template-columns: minmax(14rem, 1fr) auto auto; align-items: center; } }
</style>
