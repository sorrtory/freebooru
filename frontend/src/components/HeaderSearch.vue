...
function replaceToken(value: string) { model.value = replaceSearchToken(model.value, value); active.value = 0; open.value = value.endsWith(':') }
function complete(event: KeyboardEvent) { if (!completions.value.length) return; event.preventDefault(); replaceToken(completions.value[active.value] ?? completions.value[0]) }
function move(step: number) { if (!completions.value.length) return; open.value = true; active.value = (active.value + step + completions.value.length) % completions.value.length }
...
<template>
  <form ref="root" class="header-search" role="search" @submit.prevent="submit">
    <div class="header-search-control"><svg aria-hidden="true" viewBox="0 0 24 24"><circle cx="11" cy="11" r="6"/><path d="m16 16 4 4"/></svg><input v-model="model" type="search" role="combobox" aria-label="Search files" placeholder="Search tags" autocomplete="off" :aria-expanded="open && Boolean(completions.length || popular.length)" :aria-activedescendant="completions.length ? `header-search-option-${active}` : undefined" @focus="open = true" @input="open = true; active = 0" @keydown.tab="complete" @keydown.down.prevent="move(1)" @keydown.up.prevent="move(-1)" @keydown.escape="open = false"></div>
    <div v-if="open && (completions.length || popular.length)" class="search-popover">
      <section v-if="completions.length" role="listbox"><h2>Complete</h2><button v-for="(item, index) in completions" :id="`header-search-option-${index}`" :key="item" type="button" role="option" :aria-selected="active === index" @mouseenter="active = index" @mousedown.prevent="replaceToken(item)">{{ item }}</button></section>
